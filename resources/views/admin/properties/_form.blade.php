            ...
            <form @class(['form h-min-dvh rounded-lg text-lg shadow-lg w-auto md:mx-auto lg:w-2/3']) method="post" action="{{ ($property->exists) ? route('admin.property.update', $property) : route('admin.property.store') }}" name="property">
                ...
                <div @class(['md:grid grid-cols-2 md:justify-between md:gap-4 lg:w-md'])>
                    <div @class(['flex flex-col w-full md:grid grid-cols-1 mb-4'])><!--md:grid grid-cols-1-->
                        ...
                        <input @class(['form-control']) id="title" name="title" type="text" value="{{ __( @old('title', $property->title)) }}" placeholder="entrer un titre">
                ...
                <div @class(['form-group sm:grid-cols-1 md:justify-between md:gap-4 lg:w-md'])>
                    <div @class(['flex flex-col w-full md:grid grid-cols-1 mb-4'])><!--sm:flex flex-wrap mb-4 sm:justify-between-->
                        <label @class(['label text-indigo-700']) for="description">Description(s)</label>
                        <textarea id="description" name="description" @class(['form-control']) cols="2" placeholder="decrire le bien dans vos mots">
                            {{ __(@old('description', $property->description)) }}
                        </textarea>
                        <p @class(['text-sm text-red'])>
                            @error('description')
                            {{$message}}
